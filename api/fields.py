...
class PhoneField(forms.CharField):
    """ 
    A Nigerian Mobile Phone Field in the format 070xxxxxxxx
    """
    default_error_messages = {
        'required': _(u'Please enter a credit card number.'),
        'invalid': _(u'The phone number you entered is an invalid Nigerian Number'),
    }
    widget = forms.TextInput(
        attrs={'placeholder': '08XXXXXXXXX', 'maxlength':11, 'minlength':11})
    ...
    def clean(self, value):
        ...
        if value[1:4] not in telcoPrefixes:
...
class ExpiryDateField(forms.MultiValueField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        months = [(x, '%02d (%s)' % (x, date(2000, x, 1).strftime(MONTH_FORMAT)))
                  for x in range(1, 13)]
        years = [(x, x) for x in range(today.year, today.year + 15)]
        fields = (
            forms.ChoiceField(choices=months, error_messages={
                              'invalid': error_messages['invalid_month']}),
            forms.ChoiceField(choices=years, error_messages={
                              'invalid': error_messages['invalid_year']}),
        )
        super(ExpiryDateField, self).__init__(fields, *args, **kwargs)
        self.widget = ExpiryDateWidget(
